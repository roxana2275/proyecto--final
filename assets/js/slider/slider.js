//mueve el slider hacia adelante, desplazando el contenido horizontalmente dentro de un contenedor.
//Selector del elemento padre que contiene el contenido del slider
//Selector de los elementos hijos dentro del elemento padre que constituyen los elementos del slider.
//Índice del elemento padre dentro de NodeList devuelto por document.querySelectorAll(parentElement).
const forward = (parentElement, child,index) => {
  ...
  export const slider = ()=>{
    document.querySelectorAll('.avancar').forEach((el,index)=>{
      el.addEventListener('click', () => forward(".cards", ".content--card",index));
    })
    document.querySelectorAll('.voltar').forEach((el,index)=>{
    el.addEventListener('click', () => backward(".cards", ".content--card",index))
    })
  }
