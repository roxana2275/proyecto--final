...
const getUsuarioById = (req, res) => {
  ...
  const mensajeSql = `
    ...
    `;
  const respuestaSql =
    "SELECT * FROM mensajes WHERE receptor_id = ? ORDER BY fecha_envio DESC";

  db.query(usuarioSql, [usuarioId], (err, usuarioResults) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Error en la base de datos al obtener usuario" });
    }
    ...
    db.query(productosSql, [usuarioId], (err, productosResults) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Error en la base de datos al obtener productos" });
      }
      ...
      db.query(carritoSql, [usuarioId], (err, carritoResults) => {
        if (err) {
          return res
            .status(500)
            .json({ error: "Error en la base de datos al cargar el carrito" });
        }
        ...
        db.query(mensajeSql, [usuarioId], (err, mensajeResults) => {
          if (err) {
            return res
              .status(500)
              .json({ error: "Error en la base de datos al cargar mensajes" });
          }

          db.query(respuestaSql, [usuarioId], (err, respuestaResults) => {
            if (err) {
              return res
                .status(500)
                .json({ error: "Error en la base al cargar mensajes" });
            }
...
const updateUsuarioById = (req, res) => {
  ...
  const sql =
    "UPDATE usuarios SET nombre = ?, apellido = ?, email = ?, telefono = ?, calle = ?, numero = ?, piso = ?, departamento = ?, localidad = ? WHERE usuario_id = ?";
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Error en la base de datos al actualizar usuario" });
      }
  ...
  try {
    ...
    db.query(checkEmailQuery, [email], async (err, results) => {
      if (err) {
        return res
          .status(500)
          .json({ error: "Error en la base de datos al verificar email " });
      }
      if (results.length > 0) {
      ...
      db.query(
        ...
        (err, result) => {
          if (err) {
            return res
              .status(500)
              .json({ error: "Error al guardar en la base " + err.message });
          }
  ...
  db.query(query, [email], async (err, results) => {
    ...
    if (results.length > 1) {
      return res
        ...
        .json({
          error:
            "Hay un inconveniente con su registro por favor contactese con el administrador",
        });
    }
    ...
    const token = jwt.sign({ userId: user.usuario_id }, process.env.secretKey, {
      ...
      db.query(
        ...
        (err, resultImg) => {
          ...
          const updatePublicacion =
            "UPDATE publicaciones SET imagenes_id = ? WHERE publicacion_id = ?";
          db.query(
            updatePublicacion,
            ...
            (err, resultUpdate) => {
              if (err) {
                 return res
                  ...
                  .json({
                    error:
                      "Error al actualizar publicación nueva: " + err.message,
                  });
              } else {
